...
def process_raw_data(raw_dir, processed_dir):
    ...
    class_to_replace = {'unknown': -1.0, 1: 1.0, 2: 0.0, '1': 1.0, '2': 0.0 }
    for key, value in class_to_replace.items():
        id_label['class'] = id_label['class'].replace(key, value)
    id_label = pandas.concat([oid_nid['newId'], id_label['class']], axis=1)
    ...
    # Map id in edgelist to newId, and add b timestamp to each edge.
